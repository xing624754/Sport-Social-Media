...
export default function MyActivities({ currentUser }) {
  ...
  const loadActivities = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/activities?tab=my");
      const data = await response.json();
      if (response.ok) {
    ...
    <div className="activitiesContainer">
      ...
      <div className="activitiesHeader">
        <div className="activitiesHeaderLeft">
          <h1>My Hosted Activities</h1>
          <p>Manage matches you have hosted, check requests, and coordinate games.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="emptyActivities">
          ...
          <p>You haven't hosted any activities yet. Click "Host Activity" to create one!</p>
        </div>
      ) : (
          ...
          {activities.map((activity) => (
            <div key={activity.activity_id} className="activityCard">
              <div className="activityCardHeader">
                ...
                {activity.my_status === "Expired" ? (
                  <span className="activityStatusBadge expired">Expired</span>
                ) : (
